...
> - **Codespaces option** — use `../codespaces/lab-guide.md` + `../codespaces/prerequisites.md` (or `../codespaces/`)
>
...
```
   Expected: a JSON response with `"version":"1.0"` and a `"served_by"` hostname, then `ok`. **Remember `served_by`** — it becomes the star of Lab 2.
...
```bash
for i in $(seq 1 8); do curl -s localhost:$PORT | grep -o '"served_by":"[^"]*"'; done
```
...
6. **⭐ Prove it to users:**
```bash
for i in $(seq 1 6); do curl -s localhost:$PORT | grep -oE '"(version|served_by)":"[^"]*"' | paste -d' ' - -; done
```
...
curl -s localhost:$PORT | grep -o '"version":"[^"]*"'
```
...
- Recordings in `videos/` (`lab1/2/3-*.mp4`) — and the same files at repo root — mirror this guide step-for-step. Play a lab's video first, then release participants to reproduce it, or share videos for self-paced follow-along.
- **Codespaces twin package:** `../codespaces/` (guides + Codespaces-native walkthrough videos).
