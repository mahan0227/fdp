...
> **Lab environment option A — Killercoda.** Same Labs 1 → 2 → 3 as the Codespaces option. If you chose **GitHub Codespaces** instead, stop and use `prerequisites-codespaces.md` (see also `prerequisites-chooser.md`).
...
One microservice, end to end across **three labs**: **Lab 1** build & containerize a web portal + JSON API → **Lab 2** run replicated on a 2-node Kubernetes cluster (curl `/api` and open the portal in your browser) → **Lab 3** deliver version 2.0 through a CI/CD pipeline into that same cluster, live. All in the browser, all free. (~70–80 minutes hands-on.)
