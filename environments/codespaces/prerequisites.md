...
> **Lab environment option B — GitHub Codespaces.** Same Labs 1 → 2 → 3 as the Killercoda option. If you chose **Killercoda** instead, stop and use `prerequisites.md` (see also `prerequisites-chooser.md`).
...
One microservice, end to end across **three labs**: **Lab 1** build & containerize a web portal + JSON API → **Lab 2** run replicated on kind (curl `/api` + Codespaces port-forward to open the portal) → **Lab 3** deliver version 2.0 through a CI/CD pipeline into that same cluster, live. All in the browser, all free. (~70–80 minutes hands-on.)
